  ...
  async createStorageInstance(params) {
    ...
    const storageInstance = {
      ...
      bulkWrite: async (documentWrites) => {
        ...
        for (const writeRow of documentWrites) {
          try {
            ...
            const existingDoc = db.prepare(`SELECT id FROM ${tableName} WHERE id = ?`).get(id);
      ...
      query: async (preparedQuery) => {
        ...
        const rows = db.prepare(`SELECT data FROM ${tableName} WHERE _deleted = 0`).all();
      ...
      // Count documents
      count: async () => {
        const count = db.prepare(`SELECT COUNT(*) as count FROM ${tableName} WHERE _deleted = 0`).get().count;
