...
describe("RxDB SQLite Database Map Tests", () => {
  ...
  it("should maintain separate database instances in the map", async () => {
    ...
    assert.strictEqual(
      sqliteDb1,
      sqliteDb1Again,
      "Should get the same db1 instance"
    );
    assert.strictEqual(
      sqliteDb2,
      sqliteDb2Again,
      "Should get the same db2 instance"
    );

    // Check that the database map contains both databases
    const availableDatabases = getRxStorageSQLite.getAvailableDatabases();
    assert.ok(
      availableDatabases.includes("db1"),
      "Available databases should include db1"
    );
    assert.ok(
      availableDatabases.includes("db2"),
      "Available databases should include db2"
    );
    assert.strictEqual(
      availableDatabases.length,
      2,
      "Should have exactly 2 databases in the map"
    );
    ...
    const db1ProductsQuery = sqliteDb1
      .prepare(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='db2_products'"
      )
      .all();
    const db2UsersQuery = sqliteDb2
      .prepare(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='db1_users'"
      )
      .all();

    assert.strictEqual(
      db1ProductsQuery.length,
      0,
      "db1 should not have db2_products table"
    );
    assert.strictEqual(
      db2UsersQuery.length,
      0,
      "db2 should not have db1_users table"
    );
    ...
    try {
      ...
      assert.ok(
        error,
        "Should throw an error when querying db2 tables from db1"
      );
    }

    // Test that we can get the database instances by name
    const db1Again = getRxStorageSQLite.getDBByName("db1");
    const db2Again = getRxStorageSQLite.getDBByName("db2");
    assert.strictEqual(db1Again, sqliteDb1, "Should get the same db1 instance");
    assert.strictEqual(db2Again, sqliteDb2, "Should get the same db2 instance");
  });
  ...
  it("should handle database destruction correctly", async () => {
    ...
    assert.ok(
      getRxStorageSQLite.getDBByName("db1"),
      "db1 should be in the map"
    );
    assert.ok(
      getRxStorageSQLite.getDBByName("db2"),
      "db2 should be in the map"
    );
    ...
    assert.ok(
      getRxStorageSQLite.getDBByName("db1"),
      "db1 should still be in the map after destruction"
    );
    ...
    assert.ok(
      getRxStorageSQLite.getDBByName("db2"),
      "db2 should still be in the map"
    );
  });
});
