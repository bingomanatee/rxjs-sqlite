...
describe('RxDB Mixed SQLite Adapters Test', () => {
  ...
  it('should maintain separate database instances for different adapter types', async () => {
    ...
    const relationalSqliteDb = getRelationalRxStorageSQLite.getDBByName('relational-db');
    ...
    assert.notStrictEqual(standardSqliteDb, relationalSqliteDb, 'Should have different database instances');
    ...
    const relationalSqliteDbAgain = getRelationalRxStorageSQLite.getDBByName('relational-db');
    
    assert.strictEqual(standardSqliteDb, standardSqliteDbAgain, 'Should get the same standard-db instance');
    assert.strictEqual(relationalSqliteDb, relationalSqliteDbAgain, 'Should get the same relational-db instance');
    ...
    assert.ok(standardDatabases.includes('standard-db'), 'Standard databases should include standard-db');
    assert.ok(relationalDatabases.includes('relational-db'), 'Relational databases should include relational-db');
    
    // Verify that the databases don't cross-contaminate
    assert.ok(!standardDatabases.includes('relational-db'), 'Standard databases should not include relational-db');
    assert.ok(!relationalDatabases.includes('standard-db'), 'Relational databases should not include standard-db');
    ...
    const standardUsers = standardSqliteDb.prepare('SELECT * FROM standard-db_users').all();
    const relationalProducts = relationalSqliteDb.prepare('SELECT * FROM relational-db_products').all();

    assert.strictEqual(standardUsers.length, 1, 'standard-db should have 1 user');
    assert.strictEqual(relationalProducts.length, 1, 'relational-db should have 1 product');
    ...
    const standardTableInfo = standardSqliteDb.prepare("PRAGMA table_info(standard-db_users)").all();
    const standardDataColumn = standardTableInfo.find(col => col.name === 'data');
    ...
    const relationalTableInfo = relationalSqliteDb.prepare("PRAGMA table_info(relational-db_products)").all();
    ...
    assert.ok(relationalColumns.includes('id'), 'Relational adapter should have an id column');
    assert.ok(relationalColumns.includes('name'), 'Relational adapter should have a name column');
    assert.ok(relationalColumns.includes('price'), 'Relational adapter should have a price column');
    
    // The relational adapter should NOT have a data column (data is stored in individual columns)
    assert.ok(!relationalColumns.includes('data'), 'Relational adapter should not have a data column');

    // Test that lastDB points to the most recently created database for each adapter
    const standardLastDb = getRxStorageSQLite.getLastDB();
    const relationalLastDb = getRelationalRxStorageSQLite.getLastDB();
    
    assert.strictEqual(standardLastDb, standardSqliteDb, 'Standard lastDB should point to standard-db');
    assert.strictEqual(relationalLastDb, relationalSqliteDb, 'Relational lastDB should point to relational-db');
  });
});
